eagle.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousEagle: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var eagleSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var eaglePromise = self._getEagleSuggestions(value, isVAT).then(function (suggestions){
                eagleSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with Eagle result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = eagleSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            eagleSuggestions.push(suggestion);
                        }
                    });
                }

                eagleSuggestions = _.filter(eagleSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(eagleSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(eagleSuggestions);
            };

            self._whenAll([eaglePromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use Eagle Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getEagleSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousEagle.add(def);
    },
    /**
