...
__author__ = eagle.release.author
__version__ = eagle.release.version

# Also use the `eagle` logger for the main script.
_logger = logging.getLogger('eagle')
...
def report_configuration():
    ...
    """
    config = eagle.tools.config
    _logger.info("Eagle version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', eagle.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = eagle.tools.config
    if not eagle.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = eagle.modules.registry.Registry.new(dbname)
        with eagle.api.Environment.manage():
            with registry.cursor() as cr:
                eagle.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with eagle.api.Environment.manage():
        with registry.cursor() as cr:
            eagle.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    eagle.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                eagle.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except eagle.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        eagle.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = eagle.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the eagle server (default command)"""
    def run(self, args):
        main(args)
