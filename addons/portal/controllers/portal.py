...
from eagle import fields as eagle_fields, http, tools, _, SUPERUSER_ID
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': eagle_fields.Date.to_string(eagle_fields.Date.from_string(date_begin)),
                'date_end': eagle_fields.Date.to_string(eagle_fields.Date.from_string(date_end)),
                'name': label,
