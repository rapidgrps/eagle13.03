...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('eagle.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
DEFAULT_LOG_CONFIGURATION = [
    'eagle.http.rpc.request:INFO',
    'eagle.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['eagle:DEBUG', 'eagle.sql_db:INFO', 'eagle.http.rpc:DEBUG'],
    'debug_rpc': ['eagle:DEBUG', 'eagle.sql_db:INFO', 'eagle.http.rpc.request:DEBUG'],
    'debug': ['eagle:DEBUG', 'eagle.sql_db:INFO'],
    'debug_sql': ['eagle.sql_db:DEBUG'],
    'info': [],
    'warn': ['eagle:WARNING', 'werkzeug:WARNING'],
    'error': ['eagle:ERROR', 'werkzeug:ERROR'],
    'critical': ['eagle:CRITICAL', 'werkzeug:CRITICAL'],
}
