...
from eagle.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if eagle.tools.config['db_host']:
        env['PGHOST'] = eagle.tools.config['db_host']
    if eagle.tools.config['db_port']:
        env['PGPORT'] = str(eagle.tools.config['db_port'])
    if eagle.tools.config['db_user']:
        env['PGUSER'] = eagle.tools.config['db_user']
    if eagle.tools.config['db_password']:
        env['PGPASSWORD'] = eagle.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    ...
    """
    import eagle.modules as addons
    adps = eagle.addons.__path__
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    import eagle.modules as addons
    paths = eagle.addons.__path__ + [config['root_path']]
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = eagle.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('eagle.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('eagle.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = eagle.fields.Datetime.from_string(value)
            value = eagle.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = eagle.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = eagle.fields.Datetime.from_string(value)
    else:
        timestamp = value
