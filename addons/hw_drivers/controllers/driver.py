...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'eagle/addons/hw_drivers/drivers'
    ...
    def send_alldevices(self):
        ...
        server = helpers.get_eagle_server_url()
        if server:
            subject = helpers.read_file_first_line('eagle-subject.conf')
            if subject:
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the eagle server with the updates
        """
