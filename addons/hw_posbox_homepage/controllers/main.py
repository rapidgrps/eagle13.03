...
import os
from eagle.tools import misc
from pathlib import Path

from uuid import getnode as get_mac
from eagle.addons.hw_proxy.controllers import main as hw_proxy
from eagle.addons.web.controllers import main as web
from eagle.modules.module import get_resource_path
from eagle.addons.hw_drivers.tools import helpers
from eagle.addons.hw_drivers.controllers.driver import iot_devices
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('eagle.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('eagle-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_eagle_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'eagle-remote-server.conf'
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "Eagle's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('eagle-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('eagle-enterprise-code.conf'),
        })
    ...
    def clear_credential(self):
        helpers.unlink_file('eagle-db-uuid.conf')
        helpers.unlink_file('eagle-enterprise-code.conf')
        subprocess.check_call(["sudo", "service", "eagle", "restart"])
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('eagle-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_eagle_server_url()
    ...
    def server(self):
        return server_config_template.render({
            ...
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_eagle_server_url() or 'Not configured yet',
    ...
    def add_six_payment_terminal(self, terminal_id):
        helpers.write_file('eagle-six-payment-terminal.conf', terminal_id)
        subprocess.check_call(["sudo", "service", "eagle", "restart"])
        return 'http://' + helpers.get_ip() + ':8069'

    @http.route('/six_payment_terminal_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_six_payment_terminal(self):
        helpers.unlink_file('eagle-six-payment-terminal.conf')
        subprocess.check_call(["sudo", "service", "eagle", "restart"])
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
