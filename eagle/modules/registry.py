...
from eagle.sql_db import TestCursor
from eagle.tools import (assertion_report, config, existing_tables, ignore,
                        lazy_classproperty, lazy_property, OrderedSet)
from eagle.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # eagle.service.wsgi_server.application
        ...
        with cls._lock:
            with eagle.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        eagle.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = eagle.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = eagle.modules.db.has_unaccent(cr)
            if eagle.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = eagle.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context):
        ...
        env = eagle.api.Environment(cr, SUPERUSER_ID, context)
