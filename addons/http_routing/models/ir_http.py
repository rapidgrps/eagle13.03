...
import eagle
from eagle import api, models, registry, exceptions
from eagle.addons.base.models.ir_http import RequestUID, ModelConverter
from eagle.addons.base.models.qweb import QWebException
from eagle.http import request
from eagle.osv import expression
from eagle.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if eagle._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            eagle._geoip_resolver = GeoIPResolver.open(geofile) or False
        except Exception as e:
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if eagle._geoip_resolver and request.httprequest.remote_addr:
                record = eagle._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except eagle.exceptions.MissingError:
