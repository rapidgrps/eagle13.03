...
``banner_route``
    ...
    .. code-block:: python
        ...
        class MyController(eagle.http.Controller):
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~eagle.fields.Char`,
    :class:`~eagle.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`eagle.api.onchange` on the model
  ...
  ``mode``
    for :class:`~eagle.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~eagle.fields.Char` field stores a password and
...
``examples``
  if set to a key in the `KanbanExamplesRegistry`_, examples on column setups will be available in the grouped kanban view. `Here <https://github.com/eagle/eagle/blob/99821fdcf89aa66ac9561a972c6823135ebf65c0/addons/project/static/src/js/project_task_kanban_examples.js#L27>`_ is an example of how to define those setups.
``group_create``
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~eagle.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/eagle/eagle/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``context``
...
``aggregate``
    ...
    - ``group_operator`` (optional)
        ...
        .. note:: The special aggregate function ``count_distinct`` (defined in eagle) can also be used here
...
``field``
    ...
    ``context``
        ...
        values e.g. ``[id_1, id_2]`` for a :class:`~eagle.fields.Many2one` field).
    ...
    ``domain``
        ...
        (e.g. :class:`~eagle.fields.Many2one`), filters the possible
        completion results.
...
.. warning::
   ...
   The Map view is only available in Eagle Enterprise
...
.. _KanbanExamplesRegistry: https://github.com/eagle/eagle/blob/99821fdcf89aa66ac9561a972c6823135ebf65c0/addons/web/static/src/js/views/kanban/kanban_examples_registry.js
