...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if not isinstance(in_self, eagle.models.BaseModel):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all eagle models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non eagle model from the log: [None])
        ...
        .. code-block:: python
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @profile()                  # log all methods for all eagle models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/openerp/eagle/eagle/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _eagleProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_eagleProfile, method)

    wrapper = decorator(_eagleProfile, method)
    return wrapper
