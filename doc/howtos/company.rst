...
.. warning::
    ...
    This tutorial requires good knowledge of Eagle.
...
To correctly manage multi-company behaviors, Eagle's ORM provides multiple features:
...
.. note:: The `_compute_display_info` method is decorated with `depends_context('force_company')`
          (see :attr:`~eagle.api.depends_context`) to ensure that the computed field is recomputed
...
On each :meth:`~eagle.models.Model.create` and :meth:`~eagle.models.Model.write`, automatic checks
