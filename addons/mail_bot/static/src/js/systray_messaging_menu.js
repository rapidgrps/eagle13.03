eagle.define('mail_bot.systray.MessagingMenu', function (require) {
"use strict";
...
return MessagingMenu.include({
     ...
     * messaging menu (e.g. 'EagleBot has a request')
     *
     * @override
     * @private
     ...
     * EagleBot, and the EagleBot influences the counter by 1 when it requests
    ...
    _handleResponseNotificationPermission: function (value) {
        ...
        if (value !== 'granted') {
            this.call('bus_service', 'sendNotification', _t('Permission denied'),
                _t('Eagle will not have the permission to send native notifications on this device.'));
        }
        this._updateCounter();
    },
    /**
    ...
    /**
     * Override so that it handles preview related to EagleBot
     *
     * @override
     * @private
     * @param {MouseEvent} ev
