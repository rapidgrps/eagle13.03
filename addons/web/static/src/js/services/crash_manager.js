eagle.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    ...
    show_error: function (error) {
        ...
        var dialog = new ErrorDialog(this, {
            title: _.str.capitalize(error.type) || _t("Eagle Error"),
        }, {
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("Eagle Warning"),
            buttons: [
...
core.crash_registry.add('eagle.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            cm.show_warning({type: _t("Eagle Session Expired"), message: _t("Your Eagle session expired. Please refresh the current web page.")});
        }
    };
}
core.crash_registry.add('eagle.http.SessionExpiredException', session_expired);
