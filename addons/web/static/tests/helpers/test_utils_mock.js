eagle.define('web.test_utils_mock', function (require) {
"use strict";
 ...
 *   used to generate mock answers for every kind of routes supported by eagle
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught Eagle
 *   events.
...
function addMockEnvironment(widget, params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            eagle.debug = params.config.debug;
        }
    }
    ...
    // make sure all Eagle events bubbling up are intercepted
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any Eagle widget)
