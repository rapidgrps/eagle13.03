...
import eagle
import eagle.modules.db
import eagle.modules.graph
import eagle.modules.migration
import eagle.modules.registry
import eagle.tools as tools

from eagle import api, SUPERUSER_ID
from eagle.modules.module import adapt_version, initialize_sys_path, load_openerp_module
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    for index, package in enumerate(graph, 1):
        ...
        if needs_update:
            ...
            if tools.config.options['test_enable']:
                ...
                report.record_result(eagle.modules.module.run_unit_tests(module_name))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, eagle.sql_db.sql_counter - t0_sql)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['eagle.addons.%s' % (pkg.name,)]
