...
import eagle
import eagle.tools as tools
import eagle.release as release
from eagle import SUPERUSER_ID, api
...
# ad_paths is a deprecated alias, please use eagle.addons.__path__
@tools.lazy
def ad_paths():
    _logger.warning('"eagle.modules.module.ad_paths" is a deprecated '
                    'proxy to "eagle.addons.__path__". Please consider '
                    'using the latter as the former is going to be '
                    'removed in the next version.',
                    exc_info=DeprecationWarning(), stack_info=True)
    return eagle.addons.__path__
hooked = False
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            _logger.warning('"openerp.addons" is a deprecated alias to '
                            '"eagle.addons". Please consider using the '
    ...
    def load_module(self, name):
        ...
        eagle_name = re.sub(r'^openerp.addons.(\w+)$', r'eagle.addons.\g<1>', name)

        eagle_module = sys.modules.get(eagle_name)
        if not eagle_module:
            eagle_module = importlib.import_module(eagle_name)

        sys.modules[name] = eagle_module
...
class EagleHook(object):
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            _logger.warning('openerp is a deprecated alias to eagle. '
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'eagle\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import eagle.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global hooked

    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in eagle.addons.__path__:
        eagle.addons.__path__.append(dd)

    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in eagle.addons.__path__:
            eagle.addons.__path__.append(ad)
    ...
    if base_path not in eagle.addons.__path__ and os.path.isdir(base_path):
        eagle.addons.__path__.append(base_path)
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    initialize_sys_path()
    for adp in eagle.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = eagle.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in eagle.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('eagle.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['eagle.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    initialize_sys_path()
    for ad in eagle.addons.__path__:
...
class EagleTestResult(unittest.result.TestResult):
    """
    ...
    def getErrorCallerInfo(self, error, test):
        ...
        # only test case should be executed in eagle, this is only a safe guard
...
def run_unit_tests(module_name, position='at_install'):
    ...
    """
    global current_test
    from eagle.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            result = EagleTestRunner().run(suite)
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, eagle.sql_db.sql_counter - t0_sql)
