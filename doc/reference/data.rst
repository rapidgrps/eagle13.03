...
.. note::
    ...
    can specify the eagle flag ``noupdate`` set to 1.  If part of
    ...
    .. code-block:: xml
      ...
      <eagle>
          <data noupdate="1">
              <!-- Only loaded when installing the module (eagle-bin -i module) -->
              <operation/>
          </data>

          <!-- (Re)Loaded at install and update (eagle-bin -i/-u) -->
          <operation/>
      </eagle>
...
``search``
    ...
    the field is a :class:`~eagle.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~eagle.fields.Many2one` and
    :class:`~eagle.fields.Reference` fields
``type``
...
.. literalinclude:: ../../eagle/addons/base/data/res.country.state.csv
    :language: text
    :lines: 1-15
...
.. csv-table::
    :file: ../../eagle/addons/base/data/res.country.state.csv
    :header-rows: 1
