...
import eagle.sql_db
import eagle.tools
from eagle.sql_db import db_connect
from eagle.release import version_info
...
def check_super(passwd):
    if passwd and eagle.tools.config.verify_admin_password(passwd):
        return True
    raise eagle.exceptions.AccessDenied()

# This should be moved to eagle.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            eagle.tools.config['load_language'] = lang
            cr.commit()

        registry = eagle.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = eagle.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = eagle.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = eagle.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = eagle.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    eagle.sql_db.close_db(db_original_name)
    db = eagle.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = eagle.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = eagle.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = eagle.tools.config.filestore(db_original_name)
    to_fs = eagle.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    eagle.modules.registry.Registry.delete(db_name)
    eagle.sql_db.close_db(db_name)
    ...
    fs = eagle.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'eagle_dump': '1',
        'db_name': cr.dbname,
        'version': eagle.release.version,
        'version_info': eagle.release.version_info,
        'major_version': eagle.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with eagle.tools.osutil.tempdir() as dump_dir:
            filestore = eagle.tools.config.filestore(db_name)
            ...
            eagle.tools.exec_pg_command(*cmd)
            if stream:
                eagle.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                eagle.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = eagle.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with eagle.tools.osutil.tempdir() as dump_dir:
        ...
        if eagle.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = eagle.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = eagle.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if eagle.tools.config['unaccent']:
                try:
...
def exp_rename(old_name, new_name):
    eagle.modules.registry.Registry.delete(old_name)
    eagle.sql_db.close_db(old_name)
    ...
    old_fs = eagle.tools.config.filestore(old_name)
    new_fs = eagle.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    eagle.tools.config.set_admin_password(new_password)
    eagle.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        eagle.tools.config['update']['base'] = True
        eagle.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@eagle.tools.mute_logger('eagle.sql_db')
...
def list_dbs(force=False):
    if not eagle.tools.config['list_db'] and not force:
        raise eagle.exceptions.AccessDenied()
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [eagle.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if eagle.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        eagle.sql_db.close_db(database_name)
...
def exp_list_lang():
    return eagle.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(eagle.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
