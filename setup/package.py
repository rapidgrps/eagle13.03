...
exec(open(os.path.join(ROOTDIR, 'eagle', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/eagle && \
    groupadd -g %(group_id)s eagle && \
    useradd -u %(user_id)s -g eagle eagle -d /var/lib/eagle && \
    mkdir /data && \
    chown eagle:eagle /var/lib/eagle /data
USER eagle
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/eagle_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False):
    ...
    run_cmd(cmd + ['%s/' % args.eagle_dir, args.build_dir])
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'eagle/addons'))
...
#  Docker stuffs
class EagleTestTimeoutError(Exception):
    pass


class EagleTestError(Exception):
    pass
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'eagle-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='eagle', exposed_port=None, detach=False, timeout=None):
    ...
    def test_eagle(self):
        logging.info('Starting to test Eagle install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'eagle.pid')):
                _rpc_count_modules(port=self.exposed_port)
                self.stop()
                return
        if self.is_running():
            self.stop()
            raise EagleTestTimeoutError('Eagle pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise EagleTestError('Error while installing/starting Eagle after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'eagle-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/eagle-*.tar.gz' % self.args.build_dir)[0], '%s/eagle_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/eagle-*.zip' % self.args.build_dir)[0], '%s/eagle_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/eagle_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s eagle"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su eagle -s /bin/bash -c "eagle -d mycompany -i base --stop-after-init"',
            'su eagle -s /bin/bash -c "eagle -d mycompany --pidfile=/data/src/eagle.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'eagle-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_eagle()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/eagle (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../eagle_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'eagle-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/eagle_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su eagle -s /bin/bash -c "eagle -d mycompany -i base --stop-after-init"',
            'su eagle -s /bin/bash -c "eagle -d mycompany --pidfile=/data/src/eagle.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'eagle-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_eagle()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'eagle-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/eagle-*.noarch.rpm' % self.args.build_dir)[0], '%s/eagle_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/eagle_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su eagle -s /bin/bash -c "eagle -c /etc/eagle/eagle.conf -d mycompany -i base --stop-after-init"',
            'su eagle -s /bin/bash -c "eagle -c /etc/eagle/eagle.conf -d mycompany --pidfile=/data/src/eagle.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'eagle-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_eagle()
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/eaglebuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing Eagle files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Eagle %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"Eagle {sv}"/python/python.exe \'c:\\Program Files\\Eagle {sv}\\server\\eagle-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def build_exe(args):
    ...
    shutil.copy(glob('%s/openerp*.exe' % args.build_dir)[0], '%s/eagle_%s.%s.exe' % (args.build_dir, VERSION, TSTAMP))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/eagle/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/eagle/vm/win1036/id_rsa', help="%(default)s")
