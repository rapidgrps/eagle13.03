...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        eagle.oepetstore = function(instance, local) {
...
method of :class:`eagle.web.Class`::
...
The :func:`~eagle.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~eagle.web.Class.extend` on the parent class, as is done
to subclass :class:`~eagle.web.Class`::
...
It extends :class:`~eagle.Widget` and overrides the standard method
:func:`~eagle.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~eagle.Widget.start`
...
All widgets have a :attr:`~eagle.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~eagle.Widget.$el` is an
empty ``<div>`` element.
...
``GreetingsWidget``'s :func:`~eagle.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~eagle.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~eagle.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~eagle.Widget.start` method will be called during the call
to :func:`~eagle.Widget.appendTo`.
...
:class:`~eagle.Widget`, because we added some classes on them.
...
When overriding the :func:`~eagle.Widget.init` method of a widget it is
...
it. This is done via the :func:`~eagle.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~eagle.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~eagle.Widget.init` or :func:`~eagle.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~eagle.Widget.destroy`.

.. danger::

    when overriding :func:`~eagle.Widget.destroy`, ``_super()``
...
However, because :class:`~eagle.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~eagle.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~eagle.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~eagle.Widget.template`. If there are
...
When using :class:`~eagle.Widget`'s integration it is not possible to
...
before :func:`~eagle.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~eagle.Widget.init`):
...
But because it's a common operation, :class:`~eagle.Widget` provides an
equivalent shortcut through the :func:`~eagle.Widget.$` method::
...
:attr:`~eagle.Widget.events` is an object (mapping) of an event to the
...
:func:`~eagle.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~eagle.Widget.on`::
...
:func:`~eagle.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~eagle.Widget.trigger` if it has
any. Example::
...
* :func:`~eagle.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~eagle.Widget.set`) and ``change``
* :func:`~eagle.Widget.get` retrieves the value of a property.
...
defined in the call to :func:`~eagle.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~eagle.Widget.include`.
...
:func:`~eagle.web._t` (and also :func:`~eagle.web._lt`).
...
:func:`~eagle.web._lt` ("lazy translate") is similar but somewhat more
...
:func:`~eagle.Widget.call` returns a deferred resolved with the value
...
solution to achieve that is to use :class:`eagle.web.CompoundContext`.

:class:`~eagle.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~eagle.web.CompoundContext` instances. It will
...
::py:meth:`~eagle.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~eagle.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`eagle.web.Query` which can be further customized before
  being executed
* :class:`~eagle.web.Query` represents the query being built. It is
  ...
  :class:`~eagle.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~eagle.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~eagle.models.Model.read`'s, an array of dictionaries where each
...
:func:`~eagle.Widget.do_action` is a shortcut of :class:`~eagle.Widget`
...
``instance.web.client_actions`` is a :class:`~eagle.web.Registry` in which
...
:py:class:`~eagle.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~eagle.web.View.view_id` attribute.

Views are also provided with a :class:`~eagle.web.DataSet` instance which
...
:func:`~eagle.web.View.do_search`, and updating their
:class:`~eagle.web.DataSet` as necessary.
...
:py:class:`~eagle.fields.Char` fields which should contain email addresses
...
their :attr:`~eagle.web.form.FormWidget.field_manager` attribute::
...
:attr:`~eagle.web.form.FormWidget` is generally the
:class:`~eagle.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~eagle.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~eagle.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~eagle.web.form.FieldManagerMixin.set_values(values)` sets multiple
