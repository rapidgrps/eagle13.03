...
from eagle.addons.payment_authorize.controllers.main import AuthorizeController
...
@eagle.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('eagle.addons.payment_authorize.models.payment', 'ValidationError')
...
@eagle.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on eagle/eagle + create sandbox account
        authorize.write({
