...
import eagle
from eagle.modules import get_modules
from eagle.modules.module import run_unit_tests, get_test_modules
from eagle.modules.registry import Registry
from eagle.release import nt_service_name
from eagle.tools import config
from eagle.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'eagle.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    # is included in all Python 3 releases installed on all operating systems supported by Eagle.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(eagle, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in eagle.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="eagle.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from eagle.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = eagle.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(eagle.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="eagle.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="eagle.service.httpd")
        t.setDaemon(True)
        t.start()
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('eagle: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(eagle.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = eagle.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('eagle.netsvc.rpc.request')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(eagle.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            eagle.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if eagle.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    result = eagle.modules.module.EagleTestRunner().run(suite)
...
def start(preload=None, stop=False):
    """ Start the eagle http server and cron processor.
    """
    global server

    load_server_wide_modules()
    eagle.service.wsgi_server._patch_xmlrpc_marshaller()

    if eagle.evented:
        server = GeventServer(eagle.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(eagle.service.wsgi_server.application)
        ...
        server = ThreadedServer(eagle.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not eagle.evented:
        if inotify:
    ...
    if getattr(eagle, 'phoenix', False):
        _reexec()
