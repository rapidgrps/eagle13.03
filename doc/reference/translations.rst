...
.. note:: If you want to contribute to the translation of Eagle itself, please refer to the
  `Eagle Wiki page <https://github.com/eagle/eagle/wiki/Translations>`_.
...
automatically loaded by Eagle when the corresponding language is installed (via
...
Eagle automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~eagle.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~eagle.models.Model._constraints` and
  :attr:`~eagle.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, Eagle cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`eagle._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`eagle.web._t`:
...
The lazy version of `_` and `_t` is :func:`eagle._lt` in python and
:js:func:`eagle.web._lt` in javascript. The translation lookup is executed only
