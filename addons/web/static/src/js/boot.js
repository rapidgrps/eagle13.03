...
(function () {
    ...
    eagle.testing = typeof QUnit === 'object';
    eagle.remainingJobs = jobs;
    eagle.__DEBUG__ = {
    ...
    eagle.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__eagle_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: eagle.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = eagle.__DEBUG__.getDependencies(job.name);
            ...
            missing = eagle.__DEBUG__.getMissingJobs();
            failed = eagle.__DEBUG__.getFailedJobs();
            ...
            if (eagle.debug || failed.length || unloaded.length) {
                ...
                if (eagle.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        eagle.__DEBUG__.jsModules = {
            missing: missing,
    ...
    eagle.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            eagle.processJobs(jobs, services);
