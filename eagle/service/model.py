...
import eagle
from eagle.exceptions import UserError, ValidationError, QWebException
from eagle.models import check_method_name
from eagle.tools.translate import translate, translate_sql_constraint
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(eagle.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if eagle.registry(dbname)._init and not eagle.tools.config['test_enable']:
                    raise eagle.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    eagle.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = eagle.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return eagle.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with eagle.registry(db).cursor() as cr:
