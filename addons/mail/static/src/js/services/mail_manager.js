eagle.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    /**
     * Get the EagleBot ID, which is the default authorID for transient messages
     *
     * @returns {Array<string>}
     */
    getEaglebotID: function () {
                ...
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        prom.then(function () {
                            ...
                            if (!self.call('bus_service', 'isEagleFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when eagle is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isEagleFocused')) {
            // no need to notify
            return;
        }
